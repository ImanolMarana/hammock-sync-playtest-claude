...
private void populateDocument(List<DocumentRevision> docList) throws DocumentStoreException {
    while (range.location < originalDocIds.size()) {
        if (processNextBatch(docList)) {
            break;
        }
        range.location = range.location + range.length;
        
        if (limitReached || !docList.isEmpty()) {
            break;
        }
    }
}

private boolean processNextBatch(List<DocumentRevision> docList) throws DocumentStoreException {
    ...
    for (DocumentRevision rev : docs) {
        if (processDocument(docList, rev)) {
            return true;
        }
    }
    return false;
...
private boolean processDocument(List<DocumentRevision> docList, DocumentRevision rev) {
    DocumentRevision innerRev = rev;
