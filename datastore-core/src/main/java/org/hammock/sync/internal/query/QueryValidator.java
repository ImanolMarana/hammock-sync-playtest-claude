...
class QueryValidator {
    ...
    for (Object fieldClause: clause) {
        ...
        if (!isValidFieldClause(fieldClause)) {
            accumulator.add(fieldClause);
            continue;
        }
        ...
        if (!fieldName.startsWith("$")) {
            predicate = processModOperator(predicate);
        } else if (predicate instanceof List) {
...
private static boolean isValidFieldClause(Object fieldClause) {
    return fieldClause instanceof Map && !((Map) fieldClause).isEmpty();
}

@SuppressWarnings("unchecked")
private static Object processModOperator(Object predicate) {
    if (predicate instanceof Map) {
        Map<String, Object> predicateMap = (Map<String, Object>) predicate;
        if(predicateMap.get(MOD) instanceof List) {
            List<Object> rawArguments = (List<Object>) predicateMap.get(MOD);
            List<Object> arguments = convertToIntegerList(rawArguments);
            predicateMap.clear();
            predicateMap.put(MOD, arguments);
        }
    }
    return predicate;
}

private static List<Object> convertToIntegerList(List<Object> rawArguments) {
    List<Object> arguments = new ArrayList<Object>();
    for (Object rawArgument: rawArguments) {
        if (rawArgument instanceof Number) {
            arguments.add(((Number) rawArgument).intValue());
        } else {
