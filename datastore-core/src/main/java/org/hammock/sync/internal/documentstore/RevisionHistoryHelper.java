...
public class RevisionHistoryHelper {
    ...
    attachments = new TreeMap<String, Attachment>(attachments);
    for (Map.Entry<String, ? extends Attachment> att : attachments.entrySet()) {
        processAttachment(att, attsMap, shouldInline, minRevPos);
    }
}

private static void processAttachment(Map.Entry<String, ? extends Attachment> att,
    LinkedHashMap<String, Object> attsMap,
    boolean shouldInline,
    int minRevPos) {
    SavedAttachment savedAtt = (SavedAttachment) att.getValue();
    ...
    try {
        if (savedAtt.revpos > minRevPos) {
            processAttachmentContent(savedAtt, theAtt, shouldInline);
            addAttachmentMetadata(savedAtt, theAtt);
        } else {
...
private static void processAttachmentContent(SavedAttachment savedAtt,
    HashMap<String, Object> theAtt,
    boolean shouldInline) throws IOException {
    if (!shouldInline) {
        theAtt.put("follows", true);
    } else {
        theAtt.put("follows", false);
        addInlineData(savedAtt, theAtt);
    }
}

private static void addInlineData(SavedAttachment savedAtt,
    HashMap<String, Object> theAtt) throws IOException {
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    OutputStream bos = Base64OutputStreamFactory.get(baos);
    InputStream fis = savedAtt.getInputStream();
    try {
        copyStreamToBase64(fis, bos);
...
private static void copyStreamToBase64(InputStream fis, OutputStream bos) throws IOException {
    int bufSiz = 1024;
...
private static void addAttachmentMetadata(SavedAttachment savedAtt,
    HashMap<String, Object> theAtt) {
