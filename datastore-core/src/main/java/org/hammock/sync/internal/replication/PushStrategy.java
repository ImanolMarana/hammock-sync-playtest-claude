...
private void validateTargetDb() throws DatabaseNotFoundException {
...
private void processBatch() throws DocumentStoreException, ExecutionException, InterruptedException, AttachmentException {
    this.state.batchCounter++;
    logBatchStart();
    long batchStartTime = System.currentTimeMillis();

    Changes changes = getNextBatch();
    int unfilteredChangesSize = changes.getResults().size();
    long lastSeq = changes.getLastSequence();

    changes = applyFilter(changes);
    int filteredChangesSize = changes.getResults().size();
    logBatchChanges(filteredChangesSize);
    ...
    updateCheckpoint(unfilteredChangesSize, lastSeq);
    logBatchComplete(batchStartTime, changesProcessed);
}

private Changes applyFilter(Changes changes) {
    if (this.filter == null) {
        return changes;
    }
    
    List<DocumentRevision> allowedChanges = new ArrayList<DocumentRevision>();
...
private void updateCheckpoint(int unfilteredChangesSize, long lastSeq) {
...
private void logBatchChanges(int filteredChangesSize) {
    String msg = String.format(
