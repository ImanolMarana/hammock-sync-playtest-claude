...
class QueryExecutor {
    ...
    if (node instanceof AndQueryNode) {
        return executeAndQueryNode((AndQueryNode) node, db);
    }
    if (node instanceof OrQueryNode) {
        return executeOrQueryNode((OrQueryNode) node, db);
    }
    if (node instanceof SqlQueryNode) {
        return executeSqlQueryNode((SqlQueryNode) node, db);
    }
    throw new QueryException("Unexpected node type " + node);
}

private Set<String> executeAndQueryNode(AndQueryNode andNode, SQLDatabase db) throws QueryException {
...
private Set<String> executeOrQueryNode(OrQueryNode orNode, SQLDatabase db) throws QueryException {
    Set<String> accumulator = null;
...
private Set<String> executeSqlQueryNode(SqlQueryNode sqlNode, SQLDatabase db) throws QueryException {
    try {
        List<String> docIds = executeDocIdRetrieval(sqlNode, db);
...
private List<String> executeDocIdRetrieval(SqlQueryNode sqlNode, SQLDatabase db) 
        throws DocumentStoreException, QueryException {
    if (sqlNode.sql != null) {
        return executeDocIdQuery(sqlNode.sql, db);
    }
    return database.getIds();
}

private List<String> executeDocIdQuery(SqlParts sqlParts, SQLDatabase db) throws QueryException {
