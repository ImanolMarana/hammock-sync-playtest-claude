...
private List<DocRevSequence> findDuplicateRevisions(SQLDatabase db) {
...
private void processRevision(SQLDatabase db, DocRevSequence lowest, Cursor c) {
    ...
    updateChildrenAndAttachments(db, lowest, lowestSequenceAsParent, lowestSequenceAsSequence, allButLowestArgs);
    handleDuplicateAttachments(db, lowest, c);
}

private void updateChildrenAndAttachments(SQLDatabase db, DocRevSequence lowest, ContentValues lowestSequenceAsParent,
                                        ContentValues lowestSequenceAsSequence, String[] allButLowestArgs) {
    ...
    int attachmentsMigrated = db.update("attachments", lowestSequenceAsSequence,
            "sequence IN " + ALL_BUT_LOWEST_WITH_ID_REV, allButLowestArgs);
...
private void handleDuplicateAttachments(SQLDatabase db, DocRevSequence lowest, Cursor c) {
    Map<String, Integer> duplicateAttachmentFilenames = findDuplicateAttachments(db, lowest, c);
    processDuplicateAttachments(db, lowest, duplicateAttachmentFilenames);
}

private Map<String, Integer> findDuplicateAttachments(SQLDatabase db, DocRevSequence lowest, Cursor c) {
    ...
    c = db.rawQuery("SELECT filename, COUNT(*) FROM attachments WHERE sequence=? " +
            "GROUP BY filename HAVING COUNT(*) > 1", new String[]{lowest.sequence.toString()});
    while (c.moveToNext()) {
...
private void processDuplicateAttachments(SQLDatabase db, DocRevSequence lowest,
                                       Map<String, Integer> duplicateAttachmentFilenames) {
    ...
    for (Map.Entry<String, Integer> duplicateAttachmentFilename : duplicateAttachmentFilenames.entrySet()) {
        deletedAttachmentCount += deleteAttachmentCopies(db, lowest, duplicateAttachmentFilename);
    }
...
private int deleteAttachmentCopies(SQLDatabase db, DocRevSequence lowest,
                                 Map.Entry<String, Integer> duplicateAttachmentFilename) {
