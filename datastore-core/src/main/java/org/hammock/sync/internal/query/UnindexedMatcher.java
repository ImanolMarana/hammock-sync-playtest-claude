...
public class UnindexedMatcher {
    ...
    if (node instanceof AndQueryNode) {
        return executeAndNode((AndQueryNode) node, rev);
    }
    if (node instanceof OrQueryNode) {
        return executeOrNode((OrQueryNode) node, rev);
    }
...
private boolean executeAndNode(AndQueryNode andNode, DocumentRevision rev) {
    boolean passed = true;
...
private boolean executeOrNode(OrQueryNode orNode, DocumentRevision rev) {
...
private boolean executeOperatorExpression(OperatorExpressionNode node, DocumentRevision rev) {
    ...
    boolean passed = handleOperatorComparison(actual, operator, expected);
    return invertResult ? !passed : passed;
}

private boolean handleOperatorComparison(Object actual, String operator, Object expected) {
