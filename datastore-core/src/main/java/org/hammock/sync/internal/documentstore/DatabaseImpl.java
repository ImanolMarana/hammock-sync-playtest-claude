...
throws ConflictException {
    try {
        DocumentRevision newWinner = prepareResolution(docId, resolver);
        if (newWinner == null) {
            return;
        }
        executeResolution(docId, newWinner);
    } catch (ExecutionException e) {
...
private DocumentRevision prepareResolution(String docId, ConflictResolver resolver) throws ExecutionException {
    ...
    DocumentRevision newWinner = resolveConflict(docTree, resolver);
    if (newWinner == null) {
        return null;
    }
    
    validateWinningRevision(newWinner);
    return newWinner;
}

private DocumentRevision resolveConflict(DocumentRevisionTree docTree, ConflictResolver resolver) {
    try {
        return resolver.resolve(docTree.getDocId(), docTree.leafRevisions(true));
    } catch (Exception e) {
...
private void validateWinningRevision(DocumentRevision newWinner) {
    if (newWinner.getRevision() == null) {
        throw new IllegalArgumentException("Winning revision must have a revision id");
    }
}

private void executeResolution(String docId, final DocumentRevision newWinner) throws ExecutionException {
    ...
    get(queue.submitTransaction(
        new SQLCallable<Void>() {
            ...
            public Void call(SQLDatabase db) throws Exception {
                executeConflictResolution(db, newWinner, revIdKeep);
                return null;
            }
        }));
}

private void executeConflictResolution(SQLDatabase db, DocumentRevision newWinner, String revIdKeep) throws Exception {
    DocumentRevisionTree docTree = get(queue.submit(new GetAllRevisionsOfDocumentCallable(newWinner.getId(), attachmentsDir, attachmentStreamFactory)));
    new ResolveConflictsForDocumentCallable(docTree, revIdKeep).call(db);
    
    if (isNewOrUpdatedRevision(newWinner)) {
        updateWinningRevision(db, newWinner);
    }
}

private boolean isNewOrUpdatedRevision(DocumentRevision newWinner) {
    if (newWinner.getClass().equals(DocumentRevision.class)) {
        return true;
    }
    if (newWinner instanceof InternalDocumentRevision) {
        InternalDocumentRevision internal = (InternalDocumentRevision)newWinner;
        return internal.isBodyModified() || 
               (internal.getAttachments() != null && internal.getAttachments().hasChanged());
    }
    return false;
}

private void updateWinningRevision(SQLDatabase db, DocumentRevision newWinner) throws Exception {
    Map<String, Attachment> attachments = newWinner.getAttachments() != null ? 
        newWinner.getAttachments() : new HashMap<String, Attachment>();
    
    Map<String, PreparedAttachment> preparedNewAttachments = 
    ...
    Map<String, SavedAttachment> existingAttachments =
        AttachmentManager.findExistingAttachments(attachments);
        
    new UpdateDocumentFromRevisionCallable(newWinner,
