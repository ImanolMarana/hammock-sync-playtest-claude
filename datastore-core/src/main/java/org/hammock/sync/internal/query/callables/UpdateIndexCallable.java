...
DocumentRevision rev) {
    ...
    processFieldNames(fieldNames, initialIncludedFields, includeFieldNames, args, rev);
    
    ContentValues contentValues = createContentValues(includeFieldNames, args);
    String tableName = QueryImpl.tableNameForIndex(indexName);
    
    return new DBParameter(tableName, contentValues);
}

private void processFieldNames(List<FieldSort> fieldNames, List<FieldSort> initialIncludedFields, 
                             List<FieldSort> includeFieldNames, List<Object> args, DocumentRevision rev) {
    for (FieldSort fieldName : fieldNames) {
...
private ContentValues createContentValues(List<FieldSort> includeFieldNames, List<Object> args) {
    ContentValues contentValues = new ContentValues();
    int argIndex = 0;
    for (FieldSort f : includeFieldNames) {
        String fieldName = String.format("\"%s\"", f.field);
        Object argument = args.get(argIndex);
        addValueToContent(contentValues, fieldName, argument);
        argIndex++;
    }
    return contentValues;
}

private void addValueToContent(ContentValues contentValues, String fieldName, Object argument) {
