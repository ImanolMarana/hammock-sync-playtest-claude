...
DocumentStoreException {
    ...
    return processBatches(batches, missingRevisions);
}

private void logChangeFeed(ChangesResultWrapper changeFeeds) {
...
private Map<String, List<String>> getMissingRevisions(ChangesResultWrapper changeFeeds) {
...
private int processBatches(List<List<String>> batches, Map<String, List<String>> missingRevisions) 
throws ExecutionException, DocumentException, DocumentStoreException {
    int changesProcessed = 0;
    
    for (List<String> batch : batches) {
        if (this.state.cancel) break;
        changesProcessed += processOneBatch(batch, missingRevisions);
    }
    
    return changesProcessed;
}

private int processOneBatch(List<String> batch, Map<String, List<String>> missingRevisions) 
throws ExecutionException, DocumentException, DocumentStoreException {
    int changesProcessed = 0;
    ...
    try {
        ...
        changesProcessed = processDocumentRevsList(result, batchesToInsert);
...
private int processDocumentRevsList(Iterable<DocumentRevsList> result, List<BatchItem> batchesToInsert) {
    int changesProcessed = 0;
    
    for (DocumentRevsList revsList : result) {
        if (this.state.cancel) break;
        
        HashMap<String[], Map<String, PreparedAttachment>> atts = processAttachments(revsList);
        if (this.state.cancel) break;
...
private HashMap<String[], Map<String, PreparedAttachment>> processAttachments(DocumentRevsList revsList) {
    HashMap<String[], Map<String, PreparedAttachment>> atts = new HashMap<String[], Map<String, PreparedAttachment>>();
    
    if (!this.pullAttachmentsInline) {
        try {
            processDocumentAttachments(revsList, atts);
        } catch (Exception e) {
            logger.log(Level.SEVERE,
                "There was a problem downloading an attachment to the datastore, terminating replication",
                e);
            this.state.cancel = true;
        }
    }
...
private void processDocumentAttachments(DocumentRevsList revsList, 
    HashMap<String[], Map<String, PreparedAttachment>> atts) throws DocumentStoreException {
    for (DocumentRevs documentRevs : revsList) {
        Map<String, Object> attachments = documentRevs.getAttachments();
        Map<String, PreparedAttachment> preparedAtts = new HashMap<String, PreparedAttachment>();
        atts.put(new String[]{documentRevs.getId(), documentRevs.getRev()}, preparedAtts);
        
        processIndividualAttachments(documentRevs, attachments, preparedAtts);
    }
}

private void processIndividualAttachments(DocumentRevs documentRevs, Map<String, Object> attachments,
    Map<String, PreparedAttachment> preparedAtts) throws DocumentStoreException {
    for (Map.Entry<String, Object> entry : attachments.entrySet()) {
        String attachmentName = entry.getKey();
        Map attachmentMetadata = (Map) entry.getValue();
        
        if (shouldProcessAttachment(documentRevs, attachmentName, attachmentMetadata)) {
            prepareAttachment(documentRevs, entry, attachmentMetadata, preparedAtts);
        }
    }
}

private boolean shouldProcessAttachment(DocumentRevs documentRevs, String attachmentName, 
    Map attachmentMetadata) throws DocumentStoreException {
...
private void prepareAttachment(DocumentRevs documentRevs, Map.Entry<String, Object> entry, 
    Map attachmentMetadata, Map<String, PreparedAttachment> preparedAtts) {
    ...
    preparedAtts.put(entry.getKey(), this.sourceDb.pullAttachmentWithRetry(
        documentRevs.getId(), 
        documentRevs.getRev(),
        entry.getKey(), 
        new AttachmentPullProcessor(this.targetDb, entry.getKey(), 
            contentType, encoding, length, encodedLength))
    );
}
