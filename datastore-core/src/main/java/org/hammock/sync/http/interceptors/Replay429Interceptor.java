...
context) {
    ...
    try {
        ...
        // We received a 429
        AtomicInteger attemptCounter = getOrInitAttemptCounter(context);
        ...
        if (attempt < numberOfReplays && context.connection.getNumberOfRetriesRemaining() > 0) {
            handleRetry(context, urlConnection, attempt);
            return context;
        } 
        return context;
    } catch (IOException e) {
...
private AtomicInteger getOrInitAttemptCounter(HttpConnectionInterceptorContext context) {
...
private void handleRetry(HttpConnectionInterceptorContext context, HttpURLConnection urlConnection, int attempt) throws IOException {
    long sleepTime = calculateSleepTime(urlConnection, attempt);
    logRetryWarning(urlConnection, sleepTime);
    performBackoffSleep(sleepTime);
...
private long calculateSleepTime(HttpURLConnection urlConnection, int attempt) {
    ...
    return processRetryAfterHeader(retryAfter, sleepTime);
}

private long processRetryAfterHeader(String retryAfter, long defaultSleepTime) {
    if (retryAfter != null) {
        try {
...
private void logRetryWarning(HttpURLConnection urlConnection, long sleepTime) throws IOException {
...
private void performBackoffSleep(long sleepTime) {
    try {
